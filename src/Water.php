...
class Water extends Images
{
    ...
    public function __construct($filename, $rootPath = null)
    {
        ...
        //字体路径
        $fontPath = dirname(dirname(dirname(__DIR__))).'/imagefont/source/font.ttc';
     ...
     * @throws Exception
     */
    public static function open($filename, $rootPath = null)
    {
        if ($filename === null || empty($filename) ||  !is_file($filename)) {
            throw new Exception('File does not exist');
        }

        return new self($filename, $rootPath);
    }
    ...
    public function save($filename = '', $image_type = null, $quality = 80)
    {
        ...
        $filename = $this->getUploadName($filename);
        //保存图像
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new Exception('Unsupported watermark location type');
                }
        }
                         ...
                         $locate = self::WATER_SOUTHEAST, $offset = 0, $angle = 0)
    {
        ...
        if(!is_file($this->fontPath)) {
            throw new Exception("Please set Font file：setFontPath()");
        }
        //获取文字信息
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new Exception('Unsupported text position type');
                }
        }
        /* 设置偏移量 */
        if (is_array($offset)) {
